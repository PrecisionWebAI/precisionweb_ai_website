...
const Page = () => {
  ...
  const selectedService: ServiceData | undefined = service.find((serviceItem) => {
    ...
    const normalizedTitle = serviceItem.title.replaceAll(" ", "-").toLowerCase();
  ...
  return (
    <>
      {selectedService ? (
        <>
          {/* <div>{selectedService.title.replaceAll("-", " ")} is my name</div> */}
          <NavbarDemo/>
          <ContainerScroll
        titleComponent={
          <ServiceHero Service_Heading={selectedService.title.replaceAll("-", " ")}  Description={selectedService.description}/>
